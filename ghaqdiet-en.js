(function () {
  ...
  function groupCard(x) {
    ...
    const contact = x.KUNTATT ? `<p class="pastoral-contact"><strong>Contact:</strong> <a href="tel:${esc(phoneHref(x.KUNTATT))}">${esc(x.KUNTATT)}</a></p>` : '';
  ...
  fetch('ghaqdiet-en.txt?v=' + Date.now(), {cache:'no-store'})
    ...
    .then(text => {
      ...
      groupsEl.innerHTML = groups.length ? groups.map(groupCard).join('') : '<p class="pastoral-note">No groups have been added yet.</p>';
      operatorsEl.innerHTML = operators.length ? operators.map(operatorCard).join('') : '<p class="pastoral-note">No pastoral teams have been added yet.</p>';
    })
    .catch(() => {
      groupsEl.innerHTML = '<p class="pastoral-note">We could not display the list at this time.</p>';
