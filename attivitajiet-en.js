    ...
    try{
      ...
      if(archiveMode){
        listEl.innerHTML=past.length?past.map(x=>x.kind==='pdf'?pdfCard(x):textCard(x)).join('')
          :'<article class="activity-card"><h2>There are no archived activities yet.</h2></article>';
      }else{
        listEl.innerHTML=future.length?future.map(x=>x.kind==='pdf'?pdfCard(x):textCard(x)).join('')
          :'<article class="activity-card"><h2>There are no announced activities yet.</h2></article>';
      }
