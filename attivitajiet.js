(function () {
  ...
  async function load(){try{
    ...
    const files=apiRes.ok?(await apiRes.json()).filter(x=>x.type==='file'&&PDF_FILE.test(x.name)).map(x=>({kind:'pdf',name:x.name,url:x.download_url||('./'+encodeURIComponent(x.name)),d:pdfDate(x.name)})):[];
    ...
    else listEl.innerHTML=future.map(x=>x.kind==='pdf'?pdfCard(x,true):textCard(x)).join('');
    await renderPdfs();
